...
public void move(int startI,int startJ,int endI,int endJ){
	...
	if(qiZi[endI][endJ]!=null&&(qiZi[endI][endJ].getName().equals("帥")||
    	...
    	//如果移动位置有棋子切不是自己的
    		try {
    			if(qiZi[endI][endJ]!=null&&qiZi[startI][startJ]!=null){
    				this.xq.cat.dout.writeUTF("<#MOVE_QIZI#>,"+
    						this.xq.cat.tiaoZhanZhe+","+startI+","+startJ+","+endI+","+endJ+","+qiZi[endI][endJ].getName()+","+qiZi[endI][endJ].getColor().getRGB()+","+endI+","+endJ+","+this.xq.userName);
    			}else if(qiZi[startI][startJ]!=null){
    				this.xq.cat.dout.writeUTF("<#MOVE_QIZI#>,"+
    						this.xq.cat.tiaoZhanZhe+","+startI+","+startJ+","+endI+","+endJ+","+","+","+","+","+this.xq.userName);
    			}
			} catch (IOException e) {
		...
		else if(qiZi[endI][endJ]!=null&&qiZi[endI][endJ].getName().equals("將")){
			jiang2_i=endI;//如果是"將"
			jiang2_j=endJ;
		}
...
public void returnmove(String userName,String tianzhanzhe){
	QzDao qzdao = new QzDao();
	List<Map<String,Object>> list =  qzdao.find(tianzhanzhe+userName);
	for(Map<String,Object> map:list){
		Object str  = map.get("dqlocation");
		if(str!=null&&str!=""){
			String[] dqlocations = (str+"").split(",");
			if(dqlocations.length==4){
				qiZi[Integer.parseInt(dqlocations[0])][Integer.parseInt(dqlocations[1])]=qiZi[Integer.parseInt(dqlocations[2])][Integer.parseInt(dqlocations[3])];
				qiZi[Integer.parseInt(dqlocations[2])][Integer.parseInt(dqlocations[3])]=null;
			}
		}
		Object targetName  = map.get("targetName");
		if(targetName!=null&&targetName!=""){
			Object targetLocation = map.get("targetLocation");
			if(targetLocation!=null&&targetLocation!=""){
				String[] targetLocations = (targetLocation+"").split(",");
				if(targetLocations.length==2){
					qiZi[Integer.parseInt(targetLocations[0])][Integer.parseInt(targetLocations[1])]=new QiZi(new Color(Integer.parseInt(map.get("targeColor")+"")),targetName+"",Integer.parseInt(targetLocations[0]),Integer.parseInt(targetLocations[1]));
				}
			}
		}
	}
	...
	public void ReturnStatus(){
		if(this.xq.caiPan){
			this.xq.returnC.setEnabled(true);
		}else{
			this.xq.returnC.setEnabled(false);
		}
	}
}
