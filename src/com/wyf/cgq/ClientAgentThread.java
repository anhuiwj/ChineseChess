...
public class ClientAgentThread extends Thread
{
	...
	public void run(){
		while(flag){
			try{
				...
				else if(msg.startsWith("<#SEND_MSG#>")){//收到对方聊天消息
					//<#SEND_MSG#>admin&&123&&
					String[] msgs = msg.split("&&");
					String newmsg = "";
					System.out.println(msg);
					if(msgs.length>1){
						for(int i=2;i<msgs.length-1;i++){
							newmsg += msgs[i];
						}
						String name = msg.replaceAll("<#SEND_MSG#>", "");
						name = name.substring(0, name.indexOf("&&"));
						msg = msg.substring(msg.indexOf("&&")+2, msg.lastIndexOf("&&"));
						this.father.textArea.append(name+"："+msg+"\r\n");
					}
				}
				...
				else if(msg.startsWith("<#IS_ALLOW#>")){//是否同意悔棋
					this.isAllow(msg);
				}
			}
	...
	public void returnMove(String msg){
		this.father.jpz.returnmove(this.father.userName,tiaoZhanZhe);
	}
	/**
	...
	public void isAllowReturn(String msg){
		//选中“是”返回0,选中“否”返回1。
		try {
			int i =  JOptionPane.showConfirmDialog(null, "是否同意悔棋");
			if(i==0){
				this.father.jpz.returnmove(this.father.userName,tiaoZhanZhe);
			}
			this.dout.writeUTF("<#IS_ALLOW#>"+tiaoZhanZhe+i);
		} catch (IOException e) {
	...
	public void isAllow(String msg){
		//选中“是”返回0,选中“否”返回1。
		try {
			String flag = msg.substring(msg.length()-1,msg.length());
			if("1".equals(flag)){
				JOptionPane.showMessageDialog(this.father,"对方拒绝悔棋","提示",
			             JOptionPane.INFORMATION_MESSAGE);//给出获胜信息
			}else{
				this.father.jpz.returnmove(this.father.userName,tiaoZhanZhe);
			}
			//this.dout.writeUTF("<#IS_ALLOW#>"+tiaoZhanZhe);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
